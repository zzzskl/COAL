...
export function initContextBuilder(refreshAll) {
  ...
  ctxAddRole.addEventListener("change", () => {
    ...
    ctxAddToolCallId.style.display = role === "tool" ? "" : "none";
    ctxAddToolCalls.style.display = role === "assistant" ? "" : "none";
  });
  ...
  ctxAddBtn.addEventListener("click", async () => {
    ...
    if (role === "assistant") {
      const tcRaw = ctxAddToolCalls.value.trim();
      if (tcRaw) {
        try {
          body.tool_calls = JSON.parse(tcRaw);
        } catch (_) {
          return alert("Invalid tool_calls JSON");
        }
      }
    }

    await fetch("/api/context/message", {
      method: "POST",
  ...
  ctxDelFileBtn.addEventListener("click", async () => {
    const name = ctxLoadSelect.value;
    if (!name) return;
    await fetch(`/api/context/file/${name}`, {
      method: "DELETE",
      headers: window.COAL.headers(),
    });
    await refreshFileList();
  ...
  async function deleteCtxMessage(index) {
    await fetch(`/api/context/message/${index}`, {
      method: "DELETE",
      headers: window.COAL.headers(),
    });
    await refreshAll();
  ...
  async function startEdit(index, m) {
    ...
    item.innerHTML = `
      <div class="ctx-edit-form">
        ...
        <input type="text" class="ctx-edit-tool-call-id" placeholder="tool_call_id" value="${escapeHtml(toolCallId)}" style="display:${m.role === 'tool' ? '' : 'none'}">
        <textarea class="ctx-edit-tool-calls" placeholder="tool_calls JSON" rows="4" style="display:${m.role === 'assistant' ? '' : 'none'}">${escapeHtml(tcJson)}</textarea>
        <textarea class="ctx-edit-content" rows="3">${escapeHtml(m.content ?? "")}</textarea>
    ...
    item.querySelector(".ctx-edit-role").addEventListener("change", (e) => {
      item.querySelector(".ctx-edit-tool-call-id").style.display =
        e.target.value === "tool" ? "" : "none";
      item.querySelector(".ctx-edit-tool-calls").style.display =
        e.target.value === "assistant" ? "" : "none";
    });
    ...
    item.querySelector(".ctx-save-btn").addEventListener("click", async () => {
      ...
      if (role === "assistant" && tcRaw) {
        try { body.tool_calls = JSON.parse(tcRaw); } catch (_) {}
      }
    ...
    item.querySelector(".ctx-cancel-btn").addEventListener("click", () => {
      renderCtxList(_lastMessages);
    });
  }
  ...
  function renderCtxList(messages) {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      div.innerHTML = `
        ...
        <span class="ctx-preview">${escapeHtml((m.content ?? "").slice(0, 40))}${(m.content ?? "").length > 40 ? "..." : ""}<span class="ctx-extra">${extra}</span></span>
  ...
  async function refreshFileList() {
    try {
      ...
      for (const f of data.files) {
        ctxLoadSelect.innerHTML += `<option value="${escapeHtml(f)}">${escapeHtml(f)}</option>`;
      }
    } catch (_) {}
  }
