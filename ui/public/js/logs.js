...
export function initLogs() {
  ...
  async function refreshLogs() {
    try {
      ...
      for (const entry of data.entries.slice(-30).reverse()) {
        ...
        div.textContent = `${time} ${entry.level.toUpperCase()} ${entry.message}`;
  ...
  function renderSnapshot(data, container) {
    ...
    const msgPreview = msgs
      .map((m, i) => {
        ...
        return `<div class="snap-line"><span class="snap-idx">${i}</span> <b>${m.role}</b> ${escapeHtml(content)}${extra}</div>`;
      })
      .join("");
    ...
    const toolList =
      tools.length > 0
        ? tools
            .map((t) => `<div class="snap-line">• <b>${escapeHtml(t.function.name)}</b> — ${escapeHtml(t.function.description || "(no description)")}</div>`)
            .join("")
    ...
    const logLines = logs
      ...
      .map(
        (e) =>
          `<div class="snap-line snap-log-${e.level}"><span class="snap-idx">${e.time.slice(11, 19)}</span> ${e.level.toUpperCase()} ${escapeHtml(e.message)}</div>`
      )
      .join("");
    ...
    container.innerHTML = `
      <div class="snap-toolbar">
        ...
        <span class="snap-session">session: ${escapeHtml(data.sessionId?.slice(0, 8) || "?")}...</span>
    ...
    container.querySelectorAll(".snap-section-hdr").forEach((hdr) => {
      hdr.addEventListener("click", () => {
        ...
        const body = container.querySelector(`#snap-section-${section}`);
        if (body) {
    ...
    container.querySelector(".snap-copy-btn").addEventListener("click", () => {
      navigator.clipboard.writeText(JSON.stringify(data, null, 2)).then(() => {
        const btn = container.querySelector(".snap-copy-btn");
    ...
    // Close
    container.querySelector(".snap-close-btn").addEventListener("click", () => {
      container.innerHTML = "";
  ...
  async function takeSnapshot() {
    ...
    try {
      ...
      if (!res.ok) {
        container.innerHTML = `<div class="snap-line exec-err" style="padding:12px">Server returned ${res.status}. Try restarting the server.</div>`;
        return;
      }
      const ct = res.headers.get("content-type") || "";
      if (ct.includes("text/html")) {
        container.innerHTML = `<div class="snap-line exec-err" style="padding:12px">Server returned HTML instead of JSON — the endpoint may not be registered. Try restarting the server (old process may be using port 3000).</div>`;
        return;
      }
      const data = await res.json();
      renderSnapshot(data, container);
    } catch (err) {
      container.innerHTML = `<div class="snap-line exec-err" style="padding:12px">Failed: ${escapeHtml(err.message)}</div>`;
    }
  }

  // Snapshot button (already in HTML)
  const snapBtn = $("#logs-snap-btn");
  snapBtn.addEventListener("click", takeSnapshot);

  // Create snapshot container below logs list
  ...
  let _intervalId = setInterval(refreshLogs, 3000);
  let _paused = false;
  ...
  pauseBtn.addEventListener("click", () => {
    ...
    if (_paused) {
      ...
      _intervalId = setInterval(refreshLogs, 3000);
      pauseBtn.textContent = "⏸";
